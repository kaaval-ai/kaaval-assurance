...
- **AMD GPU Mode** — Gemma on ROCm + vLLM on the measured AMD hackathon host. The committed probe, telemetry, and trajectory bundle supplies the evidence.
...
5. **AMD Proof Run** — the runtime probe records GPU/runtime facts (rocm-smi, vLLM version, served model id, model family), Gemma serves through ROCm + vLLM, the eval writes telemetry artifacts, and the hosted demo replays the committed measured bundle.

## Why this is an inference assurance plane, not a debate bot

No model argues with another model in the request path. Layer 1 is deterministic code; escalation is verifier-gated, not vibes-gated; drift tracking is arithmetic over stored rows; and the only adversarial component is offline, sampled, and calibration-gated. The product is governed inference with an evidence trail — replayable trajectories, source-tagged claims, and a cost per contract-conformant answer.

## Built vs pending

**Built and tested:** provider-neutral runtime with explicit provider switching (mock / Ollama / vLLM / Fireworks), task contracts, Layer 1 verifier, Layer 2 EWMA + closed-loop routing, Layer 3 sampled audit with FP-only calibration, trajectory store, telemetry truth layer, runtime probe, eval CLI, Streamlit demo console, and the multi-step agent workflow.

**Measured proof available:** the committed July 10 bundle records Gemma 3 1B through ROCm + vLLM on an AMD/ATI `gfx1100` host and is classified `MEASURED AMD RUN` only after runtime, model, and trajectory coherence checks pass. This proves the execution and Layer-1 conformance path, not general semantic accuracy. Future deployments must capture a fresh probe and coherent bundle before inheriting that label.

## How this supports Track 3 judging

This walkthrough gives judges the architecture without requiring a live model, and every animated claim maps to a mechanism in `src/kaaval_assurance/` with a stored telemetry field behind it. AMD usage is demonstrated by the committed probe, eval telemetry, trajectory, and checksums.
